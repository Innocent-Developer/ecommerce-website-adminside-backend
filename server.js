...
function generateEmailTemplate(order) {
  return `
    <div style="max-width: 600px; margin: 20px auto; padding: 20px; background: linear-gradient(to bottom right, #f0fdf4, #d1fae5); border-radius: 20px; box-shadow: 0 10px 25px rgba(0, 0, 0, 0.1); font-family: Arial, sans-serif; color: #1f2937;">
      ...
      <img src="${
        order.productImage
      }" alt="Product Image" style="display: block; margin: 20px auto; border-radius: 10px; max-width: 100%; height: auto;">
      ...
      <div style="margin-top: 20px; padding: 20px; background-color: #ffffff; border-radius: 15px; border: 1px solid #e5e7eb;">
        ...
        <p><strong style="color: #6b7280;">Order ID:</strong> <span style="color: #111827;">${
          order._id
        }</span></p>
        <p><strong style="color: #6b7280;">Product Name:</strong> <span style="color: #111827;">${
          order.productName
        }</span></p>
        <p><strong style="color: #6b7280;">Product Price:</strong> <span style="color: #111827;">$${
          order.productPrice
        }</span></p>
...
app.post("/account/login", async (req, res) => {
  try {
    ...
    const clientIP = (
      forwarded ? forwarded.split(",")[0] : req.connection.remoteAddress
    ).trim();

    // Get Address from Coordinates
    let locationDetails = "Unknown Location";
    if (latitude && longitude) {
      locationDetails = await getAddressFromCoords(latitude, longitude);
    }
...
app.post("/account/forgot-password", async (req, res) => {
  try {
    ...
    const isPrivateIP =
      /^(10\.|192\.168|172\.(1[6-9]|2[0-9]|3[0-1])|::1|127\.)/.test(clientIP);
